...
<head>
    ...
    <title>Track Application - BritFly Jobs</title>
    <!-- Bootstrap CSS -->
...
<body>
    ...
    {{-- Navbar (Simple) --}}
    <nav class="mb-5 bg-white shadow-sm navbar navbar-light">
        <div class="container">
            <a class="navbar-brand fw-bold text-primary" href="#">
                <i class="bi bi-airplane-engines-fill me-2"></i>BritFly Jobs
            </a>
            <a href="{{ route('applications.track.form') }}" class="btn btn-outline-primary btn-sm">Check Status</a>
        </div>
    </nav>
            ...
            <div class="col-md-8">
                ...
                @if(isset($applications))
                    @if($applications->count() > 0)
                        <h5 class="mb-3 text-muted">Found {{ $applications->count() }} Application(s)</h5>
                            ...
                            <div class="card-body">
                                <div class="row align-items-center">
                                    ...
                                    <div class="mb-3 text-center col-md-2 mb-md-0">
                                        @if($app->photo)
                                            <img src="{{ asset($app->photo) }}" class="shadow rounded-circle" width="60" height="60" style="object-fit:cover;">
                                        @else
                                            <img src="https://via.placeholder.com/60" class="shadow rounded-circle">
                                        @endif
                                    </div>

                                    {{-- Details --}}
                                    <div class="col-md-7">
                                        <h5 class="mb-1 text-dark fw-bold">{{ $app->name }}</h5>
                                        <p class="mb-1 text-muted small">
                                            <i class="bi bi-briefcase me-1"></i> {{ $app->job->title ?? 'N/A' }}
                                        </p>
                                        <p class="mb-0 text-muted small">
                                            <i class="bi bi-calendar3 me-1"></i> Applied at: {{ $app->created_at->format('d/m/Y') }}
                                        </p>
                                    </div>
                    ...
                    @else
                        ...
                        <div class="p-5 text-center border-0 shadow-sm card">
                            ...
                            <h4 class="mt-3 text-dark">No Application Found</h4>
                            <p class="text-muted">No application found with this phone number. Please check the number and try again.</p>
                        </div>
                    @endif
                    <div class="mt-4 text-center">
                    <a href="{{ route('applications.apply') }}" class="px-4 btn btn-outline-primary rounded-pill">
                        <i class="bi bi-arrow-left-circle me-2"></i> Return to Application Form
                    </a>
                </div>
                @endif
